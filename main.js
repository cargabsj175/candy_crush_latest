...
*/
var tasty_sound = new Audio('sounds/tasty.ogg');
var sweet_sound = new Audio('sounds/sweet.ogg')
var divine_sound = new Audio('sounds/divine.ogg')
...
// â†’â†’â†’ Function Definitions â†â†â†
		...
		return function getNewTileInternal (x,y, isBonus, type) { //x/y are in terms of tiles from the origin. type is the index of the bitmap. isBonus can be undefined or the type of bonus.
			...
			tileToReturn.remove = function(_, quietly) { //Note: Only call if you've actually added this tile to the stage. Set "quietly" to true if you're subbing out this tile for another one. If "quietly", score popup is suppressed and jelly isn't removed.
				...
				if(!quietly) {
					...
					drawTileScore(Math.floor(40 * (matchesMadeThisMove/2+1)), tileToReturn.x+tileWidth/2, tileToReturn.y+tileHeight/2, 20, ["#F8DB63", "#CF8A09"]);
				}
			...
			if(!matches.length) {
				var drawRating = function() {
					if(matchesMadeThisMove > 4) {
						drawBigOverlay('Delicious!', Width/4, Height/4, 50, ["#F8DB63", "#CF8A09"]);
						delicious_sound.play();
					} else if(matchesMadeThisMove > 3) {
						drawBigOverlay('Divine!',    Width/4, Height/4, 50, ["#F8DB63", "#CF8A09"]);
						divine_sound.play();
					} else if(matchesMadeThisMove > 2) {
						drawBigOverlay('Sweet!',     Width/4, Height/4, 50, ["#F8DB63", "#CF8A09"]);
						sweet_sound.play();
					} else if(matchesMadeThisMove > 1) {
						drawBigOverlay('Tasty!',     Width/4, Height/4, 50, ["#F8DB63", "#CF8A09"]);
						tasty_sound.play();
					}
				};
	...
	sugarRush = function() {
		...
		if(!!_.head(bonusTiles)) {
			drawBigOverlay('Sugar Rush', Width/4, Height/4, 40, ["#F8DB63", "#CF8A09"]);
